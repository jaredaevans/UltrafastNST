...
import torch
from layers import Conv, UpConv, ResLayer
...
class ImageTransformer(torch.nn.Module):
    ...
    def __init__(self,leak=0.05,
        ...
        self.down_conv = torch.nn.Sequential(
            Conv(3, filters[0], outerK, 1, DWS=DWSFL),
            norm_layer(filters[0], affine=True),
            torch.nn.LeakyReLU(leak),
            Conv(filters[0],filters[1], 3, 2, DWS=DWS),
            norm_layer(filters[1], affine=True),
            torch.nn.LeakyReLU(leak),
            Conv(filters[1], filters[2], 3, 2, DWS=DWS),
        ...
        self.up_conv = torch.nn.Sequential(
            UpConv(filters[2], filters[1], 4, 2, DWS=DWS),
            norm_layer(filters[1], affine=True),
            torch.nn.LeakyReLU(leak),
            UpConv(filters[1], filters[0], 4, 2, DWS=DWS),
            norm_layer(filters[0], affine=True),
            torch.nn.LeakyReLU(leak),
